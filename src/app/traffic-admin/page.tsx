...
import { Radio, ExternalLink, Terminal, AlertCircle, RefreshCw, Cpu, Activity, Shield } from 'lucide-react'
...
export default function TrafficAdminPage() {
  ...
  return (
    ...
    >
      {/* Premium Hero / Header */}
      <motion.div
        className="overflow-hidden rounded-2xl bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-8 text-white shadow-2xl relative"
        variants={itemVariants}
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-40">
          <div className="absolute -right-32 -top-32 h-64 w-64 rounded-full bg-indigo-500/20 blur-3xl" />
          <div className="absolute -bottom-16 -left-16 h-64 w-64 rounded-full bg-purple-500/15 blur-3xl" />
        </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold">
                ...
                LIVE MUNICIPAL LINK ACTIVE
              </div>
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight">
              Smart Traffic <span className="text-indigo-400">Admin Command</span>
            </h1>
            <p className="mt-2 text-slate-300 max-w-xl text-sm leading-relaxed">
              Integrates real-time video surveillance feeds, 168-slot autonomous parking structures, multi-agent AI neural coordination, and citizen reporting directly under one workspace.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Button
              variant="primary"
              className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold flex items-center gap-2 rounded-xl py-3 px-5 shadow-lg"
        ...
        {iframeError ? (
          <Card className="border-2 border-dashed border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-dark-900 overflow-hidden rounded-2xl shadow-lg">
            <CardContent className="p-10 flex flex-col items-center text-center">
              <div className="w-16 h-16 bg-indigo-50 dark:bg-indigo-950/50 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center mb-6 shadow-inner">
                <Terminal size={32} />
              </div>
              ...
              <div className="mt-8 w-full max-w-2xl bg-slate-900 text-slate-200 font-mono text-xs text-left p-6 rounded-2xl border border-slate-800 shadow-2xl relative">
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-indigo-400 font-bold"># Option 1: Start everything automatically (Monorepo dev command)</p>
                    <code className="block bg-slate-950 p-2.5 rounded-lg border border-slate-850 text-white mt-1 cursor-pointer select-all">
                      npm run dev:all
                    </code>
                  </div>
                  <div>
                    <p className="text-indigo-400 font-bold"># Option 2: Start traffic backend & frontend in parallel</p>
                    <code className="block bg-slate-950 p-2.5 rounded-lg border border-slate-850 text-white mt-1 cursor-pointer select-all">
                      npm run dev:traffic-all
                    </code>
                  </div>
                  <div>
                    <p className="text-indigo-400 font-bold"># Option 3: Start individual services manually</p>
              ...
              <div className="mt-8 flex gap-4">
                <Button
                  variant="primary"
                  className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl px-6 py-3"
                  onClick={handleRetry}
                >
            ...
            {!iframeLoaded && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-50 dark:bg-dark-900 z-10 p-10">
                <div className="relative mb-6">
                  ...
                  <Cpu className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-indigo-600 animate-pulse" size={24} />
                </div>
                <h4 className="text-lg font-bold text-slate-800 dark:text-white">Connecting to Traffic Control Dashboard</h4>
